...
query = ['SELECT GLCMMinValue, GLCMMaxValue from normalizedTextureResultsFinal where GLCMLimitsMethod like ''AutoROI'' and ADCMethod like ''linear[_]b0[_]1000'' and ImageXSize = 192 and ImageYSize = 192 and GLCMResolution = 32 and AddedNoiseStd = 0'];
tmp = handles.textureValidation.SqlQuery(query);
tmp = dataset2cell(tmp);
tmp2 = cellfun(@double,tmp(2:end,:));
...
%     
%     % ADC
%     tmp = handles.textureValidation.SqlQuery(sprintf('select distinct ADCMethod from %s order by ADCMethod',tableName{1}));
%     tmp = dataset2cell(tmp);
