...
result = [];
result.(variables{1}) = zeros(numel(GLCMResSettings)*numel(xResSettings)*numel(windowMethods)*numel(ADCMethods),...
    numel(noiseSettings));
            ...
            for GLCMRes = GLCMResSettings
                ...
                for noise = noiseSettings
                    ...
                    query = sprintf([query ' from ' tableName ' where '...
                        ...
                        xRes,xRes,GLCMRes,noise,ADCMethod{1},windowMethod{1});
...
save('NoiseResults.mat','result')

% Save to text files for plotting with gnuplot
vars = fieldnames(result);
for i = 1:2:numel(vars)
    dlmwrite(fullfile('C:\Plots\TextureArticle\ScatterPlots',['Noise' vars{i} vars{i+1} 'Normalized.csv']),[result.(vars{i}) result.(vars{i+1})])
end
