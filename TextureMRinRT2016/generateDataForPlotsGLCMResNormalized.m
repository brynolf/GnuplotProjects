...
result = [];
result.(variables{1}) = zeros(numel(noiseSettings)*numel(xResSettings)*numel(windowMethods)*numel(ADCMethods),numel(GLCMResSettings));
...
for i = 1:2:numel(vars)
    dlmwrite(fullfile('C:\Plots\TextureArticle\ScatterPlots',['GLCMRes' vars{i} vars{i+1} 'Normalized.csv']),[result.(vars{i}) result.(vars{i+1})])
end
